...
public class PlayerController : MonoBehaviour {
    ...
    [SerializeField] private CorvetteTurret turret;
    ...
    void Start() {
        ...
        //mainCamera = cameraPivot.transform.Find("Main Camera").GetComponent<Camera>();
    }
    ...
    void Update() {
        ...
        if (Physics.Raycast(aimRay, out RaycastHit aimHit, 500f, playerMask + (1 << 13))) {
            //Debug.Log("Raycast hit: " + aimHit.transform.name);
            if (aimHit.transform.gameObject.layer == 9 || aimHit.transform.gameObject.layer == 13) {
                CorvetteCore enemy = aimHit.transform.root.gameObject.GetComponent<CorvetteCore>();
